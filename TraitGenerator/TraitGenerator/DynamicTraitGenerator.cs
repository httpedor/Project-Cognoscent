...
public class DynamicTraitGenerator : ISourceGenerator
{
	...
	private static readonly DiagnosticDescriptor MustBePartialDescriptor = new(
		id: "TG001",
		title: "Component deve ser partial",
		messageFormat: "A classe '{0}' extende Component e precisa ser declarada como partial",
	...
	private static readonly DiagnosticDescriptor InvalidDependencyMemberDescriptor = new(
		id: "TG002",
		title: "Dependency inválida",
		messageFormat: "Em '{0}', o membro '{1}' com atributo '{2}' precisa ser gravável e ter o mesmo tipo do atributo",
		category: "TraitGenerator",
		defaultSeverity: DiagnosticSeverity.Error,
		isEnabledByDefault: true);
	...
	public void Execute(GeneratorExecutionContext context)
	{
		...
		// (2) Emitir erro se qualquer classe que extende Component não for partial
		...
		// (1) Componentes "no folder": arquivos em Rpg/Entities/Components/
		...
		INamedTypeSymbol componentSymbol)
	{
		...
		{
			// Se o tipo já declara GetId(), não geramos para evitar conflito.
			...
			// Se algum base type já tem implementação concreta, também não geramos.
		...
		// Arquivo gerado contém múltiplos namespaces.
	...
	{
		// (A) Descobrir eventos concretos (classes que herdam ComponentEvent)
		...
		// (B) Para cada componente concreto, descobrir quais eventos ele "sabe" tratar via ComponentEventHandler<T>
		// Também adicionamos os T dos handlers como "eventTypes" conhecidos, para gerar arrays/lookup mesmo se ainda
		// não existirem subclasses concretas de ComponentEvent no projeto.
		...
		foreach (var ev in orderedEvents)
		{
			...
			foreach (var comp in folderComponents)
			{
				...
				// Mapeamento é por tipo exato do handler (ex: ComponentEventHandler<FooEvent>).
				// Eventos derivados são atendidos via fallback em GetEventListenerComponentIds (subindo BaseType).
		...
		sb.AppendLine("    {");
		sb.AppendLine("        // Lookup exato + fallback para base types (permite handlers declarados para tipos base). ");
		...
		ImmutableArray<INamedTypeSymbol> folderComponents)
	{
		...
		// Mapear concretos (com ID) -> símbolo
		...
		INamedTypeSymbol optionalAttrSymbol)
	{
		...
		// Só geramos partial se ao menos um dos dois existe (mesmo que vazio, a gente pode optar por não tocar na classe).
	...
	private static void GenerateComponentIdConsts(GeneratorExecutionContext context, ImmutableArray<INamedTypeSymbol> components)
	{
		...
		// Não use file-scoped namespace aqui: o arquivo gerado contém múltiplos namespaces.
